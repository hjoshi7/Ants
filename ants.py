...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self) and place.ant.is_container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        x=self.place
        moves=0
        while not x.is_hive and moves<=self.upper_bound:
            if not x.bees or moves<self.lower_bound:
                x=x.entrance
                moves+=1
            else:
...
# END Problem 12
        ...
        while exit.exit:
            if exit.ant:
                if exit.ant.is_container:
                    if exit.ant.ant_contained:
                        Ant.double(exit.ant.ant_contained)
                Ant.double(exit.ant)
            exit=exit.exit
        # END Problem 12
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health
        if self.health<=0 or self.place.bees:
            ants_lose()
    def remove_from(self, place):
        pass
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self,target):
        if target:
            def make_slow(target,gamestate):
                if target.time>0:
                    if target.time%2!=0:
                        target.time-=1
                    else:
                    ...
                    return Bee.action(target,gamestate)
            target.time=5
            target.action=make_slow.__get__(target,Bee)
    # END Problem EC
